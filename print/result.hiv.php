...
/* MYSQL QUERIES SECTION */
	...
	$_ihead = $con->getArray("SELECT DATE_FORMAT(result_date,'%m/%d/%Y') AS rdate, b.patient_name, b.patient_address, c.gender as xgender, IF(c.gender='M','Male','Female') AS gender, FLOOR(DATEDIFF(so_date,c.birthdate)/364.25) AS age, b.physician, a.serialno, a.created_by, a.verified, a.verified_by, a.result, a.remarks, b.trace_no,a.code FROM lab_hivresult a LEFT JOIN so_header b ON a.so_no = b.so_no AND a.branch = b.branch LEFT JOIN patient_info c ON b.patient_id = c.patient_id WHERE a.so_no = '$_REQUEST[so_no]' AND `code` = '$_REQUEST[code]' AND serialno = '$_REQUEST[serialno]';");
	$b = $con->getArray("SELECT method,testkit,lotno,date_format(expiry, '%m/%d/%Y') as expiry,result,remarks FROM lab_hivresult WHERE so_no = '$_REQUEST[so_no]' AND `code` = '$_REQUEST[code]' and serialno = '$_REQUEST[serialno]';");
...
$mpdf=new mPDF('win-1252','LETTER-H','','',10,10,62,30,5,5);
...
<table width=100% cellpadding=2 cellspacing=0 style="font-size: 8.5pt;margin-top:5px;">
	<tr>
...
<table align=right width=70% cellpadding=5 style="font-size: 8pt;margin-top:5px;">
	<tr>
...
<table align=right width=100% cellpadding=5 style="font-size: 8pt;margin-top:5px;">
...
<div id="main">
	...
	<table width=100% cellpadding=0 cellspacing=0 style="font-style: italic; margin-top: 5px; font-size: 7pt;">
