...
/* MYSQL QUERIES SECTION */
    ...
    $b = $con->getArray("SELECT attribute, code, concat(`value`,unit) as val, verified, verified_by FROM lab_singleResult WHERE so_no = '$_REQUEST[so_no]' and code = '$_REQUEST[code]' and serialno = '$_REQUEST[serialno]';");	
...
$mpdf=new mPDF('win-1252','LETTER-H','','',10,10,68,30,5,5);
...
<div id="main">
    <table width=60% cellpadding=0 cellspacing=0 align=center style="border-collapse: collapse;">
            ...
            <tr>';
			if($b['code'] == 'L080') {
				$html .= '<td class="itemResult">'.$b['attribute'].'</td>
						  <td class="itemResult">'.$b['val'].'</td>
						  <td class="itemResult"><b>MALE&nbsp;:&nbsp;</b>0-15 mm/hr<br /><b>FEMALE&nbsp;:&nbsp;</b>0-20 mm/hr</td>';
			}else {
                $html .= '<td class="itemResult">'.$b['attribute'].'</td>
                		  <td class="itemResult">'.$b['val'].'</td>
                		  <td class="itemResult">'.$con->getAttribute($b['code'],$a['age'],$a['xgender']).'</td>';
			}
     $html .=  '</tr>
